...
llama_model_loader::llama_model_loader(const std::string & fname, std::vector<std::string> & splits, bool use_mmap,
                                       bool check_tensors, const llama_model_kv_override * param_overrides_p,
...
const struct ggml_tensor * llama_model_loader::check_tensor_dims(const std::string &          name,
                                                                 const std::vector<int64_t> & ne, bool required) const {
...
struct ggml_tensor * llama_model_loader::create_tensor(struct ggml_context * ctx, const std::string & name,
    ...
    if (cur == NULL) {
        fprintf(stderr, "❌ create_tensor failed: tensor [%s] not found\n", name.c_str());
        return NULL;
    }
    ...
    if (duplicated) {
        fprintf(stderr, "⚠️  [gguf::done_getting_tensors] Duplicate tensor detected: %s, size += %zu bytes\n", cur->name,
                ggml_nbytes(cur));
        size_data += ggml_nbytes(cur);
    } else {
        n_created++;
        fprintf(stderr, "✅ [create_tensor] Created tensor %3d: %s\n", n_created, name.c_str());
    }
...
struct ggml_tensor * llama_model_loader::create_tensor_as_view(struct ggml_context * ctx, struct ggml_tensor * base,
    ...
    if (cur == NULL) {
        fprintf(stderr, "❌ create_tensor_as_view failed: tensor [%s] not found\n", name.c_str());
        return NULL;
    }
    ...
    n_created++;
    fprintf(stderr, "✅ [create_tensor_view] Created tensor %3d: %s\n", n_created, name.c_str());
...
bool llama_model_loader::load_all_data(struct ggml_context * ctx, llama_buf_map & bufs, llama_mlocks * lmlocks,
